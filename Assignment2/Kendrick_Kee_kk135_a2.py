...
def question_3():
    ...
    if(age >= 55):
        # prompt for RA when user is older than 55
        ...
        if(oa > 20000):  # normalize OA to 20000 as extra interest calculations for OA is capped at 20000
            oa = 20000
        # get the total sum for extra interest computation with normalized OA
        ...
        if(total_sum >= 60000):  # if total sum is >= 60k offer maximum interest of $900
            ...
            # get 1% of the remainder of the total sum
            total_int += ((total_sum % 30000)*0.01)
        else:
    ...
    while(True):  # loop until a blank line is detected
        ...
        for i in x:
            if i == '[':  # if its an opening bracket, set flag to 1 so as no to read any chars till a closing bracket is found
                flag += 1
            # if a closing brace is found AND flag is 1 or more(nesting) reduce the flag by 1
            elif i == ']' and flag > 0:
                flag -= 1
